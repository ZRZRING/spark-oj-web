...
const filteredContests = computed(() => {
    ...
    return contestSet.value.filter((item) => {
        const hitKeyword = !keywordText
            ...
            || item.cid.toLowerCase().includes(keywordText)
...
<template>
    <el-card v-loading="loading" shadow="never" style="margin: 20px auto; max-width: 1200px;">
        ...
        <template v-else>
            <div v-for="(item, index) in filteredContests" :key="item.cid">
                        ...
                        <el-space alignment="flex-start" :size="16">
                            ...
                            <el-space direction="vertical" :size="6" alignment="flex-start">
                                <el-space :size="6" alignment="center">
                                    <router-link :to="`/contest/${item.cid}`">{{ item.title }}</router-link>
